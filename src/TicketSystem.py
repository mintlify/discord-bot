...
class TicketCommands(commands.Cog):
    ...
    @commands.slash_command(name="issue_ticket",
    ...
    async def issue_ticket(self, ctx, category: str, mentions: str) -> None:
        ...
        wordlist = open("../clean_words_alpha.txt", "r",
        ...
        while self.cursor.rowcount != 0:
            ticket_id = "".join(
                f"{random.choice(wordlist)}-" for _ in range(2))
            ticket_id = ticket_id[:-1]
            self.cursor.execute(
        ...
        if any(role.id in self.staff_roles for role in ctx.author.roles):
            ...
            try:
                channel = discord.utils.get(
                    guild.channels, name=ticket_id.lower())
            except AttributeError:
            ...
            async for message in channel.history(limit=1000):
                if message.author == self.bot.user:
                    ...
                    original_embed.set_footer(
                        text=f"Ticket claimed by: {ctx.author}")
            ...
            if opened_by == resolver:
                ...
                for message in transcript:
                    if message.embeds:
                        ...
                        participants = message.embeds[0].description.split("Participants:")[
                            1]
