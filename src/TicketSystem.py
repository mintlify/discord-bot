...
class TicketCommands(commands.Cog):
    ...
    async def opentickets(self, ctx) -> None:
        if not any(role.id in self.staff_roles for role in ctx.author.roles):
            ...
            if len(tickets) == 0:
                embed = EmbedBuilder(
                    "No Open Tickets", "There are currently no open tickets."
                ).build()
                await ctx.respond(embed=embed, ephemeral=True)
                return
            # Create embed
            ...
            for ticket in tickets:
                ...
                embed.add_field(
                    name="Ticket ID: %s | Ticket Category: %s" % (ticket[0], ticket[1]),
                    value="Ticket Opened: %s | Ticket Resolver: [%s] %s"
                    % (date, ticket[7], ticket[6]),
                    inline=True,
                )
            await ctx.respond(embed=embed, ephemeral=True)
    ...
    async def gettranscript(self, ctx, ticket_id: str) -> None:
        if any(role.id in self.staff_roles for role in ctx.author.roles):
            ...
            self.db_connection.ping()
            self.cursor.execute(
                "SELECT transcript FROM tickets WHERE id = '%s'" % ticket_id
            )
            ...
            embed = discord.Embed(
                title="Transcript",
                description="Transcript for ticket: %s" % ticket_id,
                color=0x18E299,
            )
    ...
    async def claimticket(self, ctx, ticket_id: str) -> None:
        if any(role.id in self.staff_roles for role in ctx.author.roles):
            ...
            self.db_connection.ping()
            self.cursor.execute(
                "SELECT category FROM tickets WHERE id = '%s'" % ticket_id
            )
            result = self.cursor.fetchone()
            if not result:
                embed = EmbedBuilder("Error", "This ticket does not exist.").build()
            ...
            embed = discord.Embed(
                title="Ticket Claim",
                description=f"Claimed ticket: %s" % ticket_id,
                color=0x18E299,
            )
            await ctx.respond(embed=embed, ephemeral=True)
    ...
    async def closeticket(self, ctx, ticket_id: str) -> None:
        ...
        if any(role.id in self.staff_roles for role in ctx.author.roles):
            ...
            if resolver[0] is None:
                embed = EmbedBuilder(
                    "Error", "This ticket has not been claimed."
                ).build()
                await ctx.respond(embed=embed, ephemeral=True)
                return
            ...
            embed = discord.Embed(
                title="Ticket Closed",
                description=f"Closed ticket: %s" % ticket_id,
                color=0x18E299,
            )
            await ctx.respond(embed=embed, ephemeral=True)
            ...
            embed = discord.Embed(
                ...
                % (ticket[0], ticket[7], ticket[6], ticket[8]),
                color=0x18E299,
            )
            channel = discord.utils.get(
                ctx.guild.channels,
    ...
    async def transferticket(self, ctx, ticket_id: str, role: discord.Role) -> None:
        ...
        if any(role1.id in self.staff_roles for role1 in ctx.author.roles):
            ...
            if resolver[0] != str(ctx.author):
                embed = EmbedBuilder(
                    "Error", "You are not the staff member who claimed this ticket."
                ).build()
                await ctx.respond(embed=embed, ephemeral=True)
                return
            ...
            if team[0] == role.name:
                embed = EmbedBuilder(
                    "Error", "This ticket is already claimed by this team."
                ).build()
                await ctx.respond(embed=embed, ephemeral=True)
                return
            # Get channel from the name of the channel
            ...
            embed = discord.Embed(
                ...
                description=f"Transferred ticket: %s" % ticket_id,
                color=0x18E299,
            )
            await ctx.respond(embed=embed, ephemeral=True)
            # Send new embed
    ...
    async def support(self, ctx) -> None:
        ...
        dm_embed = discord.Embed(
            title="Create Ticket", description=DM_support_message_stage1, color=0x18E299
        )
        ...
        try:
            ...
            if reaction.emoji == "1️⃣":
                ...
                # Update embed
                dm_embed = EmbedBuilder(
                    title="Ticket Creation", description="Ticket creation cancelled."
                ).build()
