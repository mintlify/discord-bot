...
class TicketCommands(commands.Cog):
    ...
    async def claimticket(self, ctx, ticket_id: str) -> None:
        if any(role.id in self.staff_roles for role in ctx.author.roles):
            ...
            try:
                ...
                await ctx.respond("Sending commands in the bot's DMs is not supported. Please"
                                  "run these in the server.", ephemeral=True)
                return
            if channel is None:
                embed = EmbedBuilder("Error", "This ticket does not exist.").build()
                await ctx.respond(embed=embed, ephemeral=True)
    ...
    async def closeticket(self, ctx, ticket_id: str) -> None:
        ...
        try:
            ...
            ctx.respond("Sending commands in the bot's DMs is not supported. Please"
                                  "run these in the server.", ephemeral=True)
        if any(role.id in self.staff_roles for role in ctx.author.roles):
            # Check if ticket exists
            self.db_connection.ping()
            ...
            if resolver[0] is None:
                embed = EmbedBuilder("Error", "This ticket has not been claimed.").build()
                await ctx.respond(embed=embed, ephemeral=True)
                return
            ...
            if closed[0] == "Y":
                embed = EmbedBuilder("Error", "This ticket has already been closed.").build()
                await ctx.respond(embed=embed, ephemeral=True)
                return
            guild = ctx.guild
            ...
            # Send user embed to alert them the ticket has been closed and who closed it
            embed = discord.Embed(title="Ticket Closed", description="Your ticket has been closed.", color=0x18e299)
            ...
            if opened_by == resolver:
                ...
                for message in transcript:
                    if message.embeds:
                        ...
                        participants = message.embeds[0].description.split("Participants:")[1]
    ...
    @commands.slash_command(name="transfer_ticket", description="Transfer a ticket to a higher level.")
    async def transferticket(self, ctx, ticket_id: str, role: discord.Role) -> None:
        ...
        if any(role1.id in self.staff_roles for role1 in ctx.author.roles):
            ...
            if resolver[0] != str(ctx.author):
                embed = EmbedBuilder("Error", "You are not the staff member who claimed this ticket.").build()
                await ctx.respond(embed=embed, ephemeral=True)
                return
            ...
            self.db_connection.commit()
            if team[0] == role.name:
                embed = EmbedBuilder("Error", "This ticket is already claimed by this team.").build()
                await ctx.respond(embed=embed, ephemeral=True)
                return
            # Get channel from the name of the channel
    ...
    async def support(self, ctx) -> None:
        ...
        try:
            ...
            if reaction.emoji == "1️⃣":
                ...
                # Update embed
                dm_embed = EmbedBuilder(title="Bug Report",
                ...
                # Update embed
                dm_embed = EmbedBuilder(title=category,
                ...
                # Update embed
                dm_embed = EmbedBuilder(title="Other",
