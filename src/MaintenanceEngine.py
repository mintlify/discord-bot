...
class MaintenanceEngine(commands.Cog):
    def __init__(self, bot: commands.Bot, db_connection) -> None:
        self.bot = bot
    ...
    async def me_scan(self, ctx: commands.Context) -> None:
        try:
            ctx.guild.channels
        ...
        if any(role.id in self.staff_roles for role in ctx.author.roles):
            ...
            for ticket in tickets:
                if ticket[1] is None:
                    self.scan_results[
                        "NULL category: %s" % tickets.index(ticket)
                    ] = ticket[0].rstrip()
                    continue
                elif ticket[1].rstrip() not in [
                    "Bug Report",
                    "Suggestion",
                    "Other",
                    "User " "Report",
                    ...
                    self.scan_results[
                        "Invalid category: %s" % tickets.index(ticket)
                    ] = ticket[0].rstrip()
                    continue
            if (
                not any(["NULL category", "Invalid category"])
                not in self.scan_results.keys()
            ):
            ...
            for ticket in tickets:
                if ticket[1] > datetime.datetime.now():
                    self.scan_results[
                        "Future opened_date: %s" % tickets.index(ticket)
                    ] = ticket[0].rstrip()
                    continue
                elif ticket[1] is None:
                    self.scan_results[
                        "NULL opened_date: %s" % tickets.index(ticket)
                    ] = ticket[0].rstrip()
                    continue
            if (
                not any(
                    ["Future opened_date", "NULL opened_date", "Invalid opened_date"]
                )
                not in self.scan_results.keys()
            ):
            ...
            for ticket in tickets:
                if ticket[0].rstrip() is None:
                    self.scan_results[
                        "NULL opened_by: %s" % tickets.index(ticket)
                    ] = ticket[0].rstrip()
                    continue
                elif len(ticket[0].rstrip()) != 18:
                    self.scan_results[
                        "Invalid opened_by: %s" % tickets.index(ticket)
                    ] = ticket[0].rstrip()
                    continue
            if (
                not any(["NULL opened_by", "Invalid opened_by"])
                not in self.scan_results.keys()
            ):
            ...
            for ticket in tickets:
                if ticket[1].rstrip() is None:
                    self.scan_results[
                        "NULL response_time: %s" % tickets.index(ticket)
                    ] = ticket[0].rstrip()
                    continue
            ...
            for ticket in tickets:
                if ticket[1].rstrip() is None:
                    self.scan_results[
                        "NULL time_to_complete: %s" % tickets.index(ticket)
                    ] = ticket[0].rstrip()
                    continue
            if not any(["NULL time_to_complete"]) not in self.scan_results.keys():
            ...
            for ticket in tickets:
                ...
                if ticket[0].rstrip().rstrip() is None:
                    self.scan_results[
                        "NULL resolver: %s" % tickets.index(ticket)
                    ] = ticket[0].rstrip()
                    continue
                elif ticket[0].rstrip()[-5] != "#" or invalidLast4Digits:
                    self.scan_results[
                        "Invalid resolver: %s" % tickets.index(ticket)
                    ] = ticket[0].rstrip()
                    continue
            if (
                not any(["NULL resolver", "Invalid resolver"])
                not in self.scan_results.keys()
            ):
            ...
            for ticket in tickets:
                if ticket[0].rstrip().rstrip() is None:
                    self.scan_results[
                        "NULL transcript: %s" % tickets.index(ticket)
                    ] = ticket[0].rstrip()
                    continue
                elif (
                    ...
                    self.scan_results[
                        "Invalid transcript: %s" % tickets.index(ticket)
                    ] = ticket[0].rstrip()
                    continue
            if (
                not any(["NULL transcript", "Invalid transcript"])
                not in self.scan_results.keys()
            ):
            ...
            for ticket in tickets:
                if ticket[0].rstrip() + ".txt" not in os.listdir(
                    os.getcwd().replace("src", "transcripts")
                ) and ticket[0].rstrip().lower() + ".txt" not in os.listdir(
                    os.getcwd().replace("src", "transcripts")
                ):
                    self.scan_results[
                        "Missing transcript: %s" % tickets.index(ticket)
                    ] = ticket[0].rstrip()
                if ticket[0].rstrip() + ".txt" not in [
                    x[0] for x in transcripts
                ] and ticket[0].rstrip().lower() + ".txt" not in [
                    y[0] for y in transcripts
                ]:
                    self.scan_results[
                        "Transcript not in database: %s" % tickets.index(ticket)
                    ] = ticket[0].rstrip()
            if (
                not any(["Missing transcript", "Transcript not in database"])
                not in self.scan_results.keys()
            ):
            ...
            for ticket in tickets:
                ...
                user = discord.utils.get(
                    self.bot.get_all_members(), name=resolver, discriminator=last4digits
                )
                if user is None:
                    self.scan_results[
                        "Resolver name change: %s" % tickets.index(ticket)
                    ] = ticket[0].rstrip()
            if not any(["Resolver name change"]) not in self.scan_results.keys():
            ...
            for ticket in tickets:
                ...
                try:
                    ...
                    self.scan_results[
                        "Ticket status mismatch: %s" % tickets.index(ticket)
                    ] = ticket[0].rstrip()
            if not any(["Ticket status mismatch"]) not in self.scan_results.keys():
                    ...
                    if value is None:
                        ...
                        match ticket.index(value):
                            case 0:
                                ...
                                self.scan_results[
                                    "NULL value found"
                                ] = "%s has a NULL value: %s, in the field %s" % (
                                    ticket[0],
                                    value,
                                    column_name,
                                )
